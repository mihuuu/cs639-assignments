...
def pad_sentences(sents, pad_id):
    ...
    """
    max_seq_length = max(len(sent) for sent in sents)
    
    aug_sents = []
    for sent in sents:
        # Add pad_id to the end of sentence
        padded_sent = sent + [pad_id] * (max_seq_length - len(sent))
        aug_sents.append(padded_sent)
